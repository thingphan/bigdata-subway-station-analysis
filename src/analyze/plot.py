...
top_n = 15
df_top = df.nlargest(top_n, 'robust_score')
...
fig.add_trace(
    go.Table(
        header=dict(
            values=["<b>정류장명</b>", "<b>행정동</b>", "<b>승하차 인원</b>", "<b>취약 지수</b>"],
...
fig.update_layout(
    ...
    title_text=f"서울시 대중교통 소외지역 핵심 정류장 (Top 100) & 상위 {top_n}개 목록",
    title_font_size=20,
