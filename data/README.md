...
| **final top 100 데이터** | `final_top100_report` | 정류소명(rankeddata.stops_nm), 행정동_명(rankeddata.adstrd_nm), X좌표(rankeddata.xcrd), Y좌표(rankeddata.ycrd), 총생활인구수(rankeddata.total_living_pop), 지하철역간의거리(rankeddata.distance_km), 일일총승하차인원(rankeddata.daily_total_on), 생활인구정규화(rankeddata.p_norm), 승하차인원정규화(rankeddata.b_norm), 지하철거리정규화(rankeddata.d_norm), 교통취약점수(robust_score) |
